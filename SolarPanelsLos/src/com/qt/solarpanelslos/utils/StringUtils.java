...
/**
	...
	/*
	 * // 防止xss bug public static String xssEncode(String s) { if (s == null ||
	 * s.isEmpty()) { return s; } StringBuilder sb = new
	 * StringBuilder(s.length() + 16); for (int i = 0; i < s.length(); i++) {
	 * char c = s.charAt(i); switch (c) { case '>': sb.append('＞');// 全角大于号
	 * break; case '<': sb.append('＜');// 全角小于号 break; case '\'':
	 * sb.append('‘');// 全角单引号 break; case '\"': sb.append('“');// 全角双引号 break;
	 * case '&': sb.append('＆');// 全角 break; case '\\': sb.append('＼');// 全角斜线
	 * break; case '#': sb.append('＃');// 全角井号 break; default: sb.append(c);
	 ...
	 * @throws Exception
	 */
	public static int getChineseStringSubLenByAsciiLen(String name,
			String endcoding, int length) throws Exception {
	 ...
	 * @throws Exception
	 */
	public static String zeroFillBySpace(String value, int len)
			throws Exception {
		if (value == null || len == 0) {
			return "";
		}
